...
private:
    ...
    consolidate_map(const shared_promise<U>& p, const attempt<V>& result, const Func& map) {
    ...
    consolidate_fmap(const shared_promise<U>& p, const attempt<V>& result, const Func& map) {
        static_assert(std::is_same<decltype(map(result.get())), future<U>>::value, 
            "cannot invoke map with a function that doesn't return future<T>");
